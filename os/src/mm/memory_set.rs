...
impl MemorySet {
    ...
    pub fn remove_framed_area(&mut self, start_va: VirtAddr, end_va: VirtAddr) -> isize {
        let mut aa: &mut MapArea =
            &mut MapArea::new(start_va, end_va, MapType::Framed, MapPermission::U);
        let mut str: bool = false;
        for area in &mut self.areas {
            if area
                .vpn_range
                .conta(VirtPageNum::from(start_va), VirtPageNum::from(end_va))
            {
                aa = area;
                str = true;
            }
        ...
        if str {
            aa.unmap(&mut self.page_table)
        } else {
            -1
        }
    }
    fn push(&mut self, mut map_area: MapArea, data: Option<&[u8]>) -> isize {
        let r = map_area.map(&mut self.page_table);
